...
Main application routes. Protected by auth check in layout. Contains DO records, items, challans.

## Key Files
- `layout.tsx` — Dashboard layout: auth guard (redirect to `/login` if unauthenticated), Navbar, footer
- `challans/page.tsx` — DO Records list: date filters, summary cards (vehicles/bags/weight), DO cards
- `challans/new/page.tsx` — New DO form: ticker, date, vehicle, DO number, direction (IN/OUT), items table, summary
- `items/page.tsx` — Item list: add/remove items that appear in DO dropdown

## Current Implementation
- DO data stored in localStorage via `lib/store.ts`
- Date range filtering with "Today" and "Clear" presets
- Summary stats: vehicle count, total bags, total weight
- DO cards show direction badge (IN/OUT with Hindi labels), vehicle, DO number, items
- Items page: simple CRUD with inline add form
- New DO form: dynamic item rows with RST number, item select, bags, weight

## What Needs Changing (per prompts)
- Migrate from localStorage to Supabase queries with RLS
- Add RBAC-aware UI (admin-only features hidden for staff)
- Implement DO edit/delete with audit logging
- Add pagination for large DO lists
- Add user management admin UI
- Add audit log viewer
- Implement real-time updates via Supabase subscriptions

## Prompts in This Directory
1. `03-delivery-order-list-filters.md` — DO list with advanced filters and pagination
2. `04-create-edit-delivery-order-form.md` — DO create/edit form with validation
3. `05-items-parties-master-data-ui.md` — Items and parties management
4. `06-admin-inventory-dashboard.md` — Admin dashboard with inventory stats
5. `07-user-management-admin-ui.md` — User management (invite, role assign, warehouse deletion)
6. `08-audit-log-viewer.md` — Audit log viewer with filters

## Related
- DO backend API: `../../api/prompts/03-delivery-orders-list-query-api.md`
- Items API: `../../api/prompts/05-items-parties-crud-api.md`
- Dashboard aggregation: `../../api/prompts/06-dashboard-aggregation-api.md`
- Database schema: `../../database/prompts/01-core-schema-migration.md`
