# deployment/ — Production Configuration & Operations

Production deployment, secrets management, monitoring, backup, and go-live checklist.

## Deployment Stack
- **Frontend**: Vercel (Next.js 16, edge functions, ISR)
- **Backend**: Supabase Cloud (PostgreSQL, Auth, Edge Functions)
- **File Storage**: Google Drive (service account, shared drives)
- **CI/CD**: GitHub Actions (lint, test, deploy)
- **Monitoring**: Vercel Analytics, Supabase Dashboard, custom health endpoints

## Key Concerns
- **Secrets**: Never commit to git. Use Vercel env vars, Supabase secrets, GitHub Actions secrets
- **RLS Review**: Verify all RLS policies before production launch
- **Backups**: Supabase daily backups + manual exports + point-in-time recovery
- **Rate Limiting**: Production-grade rate limits on auth and API endpoints
- **Caching**: CDN headers, service worker caching, browser cache strategy
- **Migration**: Dual-run from Google Sheets to web app with rollback plan

## Prompt Locations (10 prompts)
1. `01-env-secrets-management.md` — Environment variables, secret rotation, .env.example
...
## Related
- Database backup strategy: `../database/prompts/10-backup-exports-pit.md`
- API health endpoints: `../api/prompts/07-health-readiness-endpoints.md`
- CI test workflow: `../tests/prompts/05-github-actions-ci-workflow.md`
