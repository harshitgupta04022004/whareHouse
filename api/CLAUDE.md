...
Supabase Edge Functions or Next.js Route Handlers. All server-side logic: auth, RBAC, CRUD, audit, file storage.

## Architecture
- **Auth**: Supabase Auth (JWT-based, `auth.uid()` for user identity)
- **RLS**: Row Level Security at database level — API layer adds business logic on top
- **Audit**: Every mutation writes to `audit_log` with JSONB snapshot and hash chain
- **Validation**: Input validation at API layer + database constraints (defense in depth)
- **Rate Limiting**: Per-endpoint rate limits (15 login/15min, 100 API/min per user)
...
// RLS scoped query (uses current_warehouse_id() from JWT)
const { data, error } = await supabase.from('delivery_orders').select('*')
```

## Prompt Locations (12 prompts)
