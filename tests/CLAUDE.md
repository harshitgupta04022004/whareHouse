...
Comprehensive test suite covering auth, RBAC, CRUD, audit integrity, and deployment readiness.

## Test Frameworks
- **E2E**: Playwright (browser automation, accessible selectors)
- **Integration**: Vitest or Jest (API endpoint testing)
- **Unit**: Vitest (pure function testing)
- **CI**: GitHub Actions (runs on push/PR)

## Test Strategy
1. **Auth tests**: Login/signup flows, session management, error handling
2. **RLS tests**: Tenant isolation — verify warehouse A cannot see warehouse B data
3. **CRUD tests**: DO create/read/update/delete with validation
4. **RBAC tests**: Admin/Manager/Staff permission matrix
5. **Audit tests**: Append-only enforcement, hash chain integrity
6. **Aggregation tests**: Dashboard stats match expected values
7. **Rate limit tests**: Endpoints reject excess requests
8. **Drive tests**: File upload/download with mocked Google API
9. **Restore tests**: Backup restore drill automation

## Test Data
- Use dedicated Supabase test project (never production)
- Seed scripts for demo warehouses, users, and sample DOs
- Environment variables for test credentials (not hardcoded)
