...
Supabase PostgreSQL database layer. All DDL, RLS policies, triggers, views, and data integrity logic.

## Schema Overview
```
warehouses        — Multi-tenant root (id, name, slug, soft_delete)
app_users         — Links auth.users to warehouses (user_id, warehouse_id, role)
parties           — Trading parties per warehouse (name, phone, warehouse_id)
items             — Warehouse inventory items (name, count, warehouse_id)
delivery_orders   — DO records (ticker, date, vehicle, number, direction, warehouse_id)
do_items          — Line items per DO (rst, item, bags, weight, do_id)
audit_log         — Append-only log (action, actor, snapshot JSONB, prev_hash)
files             — Google Drive file metadata (drive_id, name, mime, category)
```

## Key Patterns
- **RLS**: Every table has `warehouse_id` + RLS policy using `current_warehouse_id()`
- **Audit**: Hash chain (`prev_hash` → `hash`) for tamper detection
- **Triggers**: `item_count` sync, `updated_at` timestamps
- **Views**: `product_summary`, `item_totals` for live derived data
- **Soft Delete**: Warehouses use `deleted_at` with 30-day recovery window

## Prompt Locations (10 prompts)
1. `01-core-schema-migration.md` — Core DDL: all tables, PKs, FKs, CHECKs, indexes
2. `02-app-users-auth-link.md` — app_users table, auth.users linkage, FK cascade
3. `03-rls-policies-isolation.md` — RLS policies per table, `current_warehouse_id()` helper
4. `04-audit-log-append-only.md` — Audit log table, append-only constraints
5. `05-triggers-item-count-updated-at.md` — Item count sync trigger, updated_at trigger
6. `06-product-summary-views.md` — product_summary and item_totals views
7. `07-audit-hash-chain-function.md` — Hash chain verification function
8. `08-files-table-drive-metadata.md` — Files table for Google Drive metadata
9. `09-soft-delete-warehouse-jobs.md` — Soft delete/recovery, purge scheduled job
10. `10-backup-exports-pit.md` — Backup strategy, point-in-time recovery

## Related
- Full schema SQL: `../database.md` (sections: Schema Design, SQL Migration Script)
- API layer uses these tables: `../api/prompts/`
- RLS policies consumed by frontend via Supabase client: `../src/`
